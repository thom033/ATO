...
public interface SecteurRepository extends JpaRepository<Secteur, Long> {
    @Query(value="select secteur.id_secteur as idSecteur,count(*) as nbSecteur,secteur.secteur,(COUNT(*) * 100.0 / total.nbPoste) AS pourcentage,nbPoste from poste natural join diplome join secteur_diplome as secteurDip on secteurDip.id_diplome=diplome.id_diplome join secteur on secteur.id_secteur=secteurDip.id_secteur,(SELECT COUNT(*) AS nbPoste FROM poste where extract(month from date_insertion)=6 and extract(year from date_insertion)=2024) as total where extract(month from date_insertion)=6 and extract(year from date_insertion)=2024 group by secteur.secteur,secteur.id_secteur,total.nbPoste",nativeQuery=true)    
